 ...
 * </p>
 */
public abstract class SimpleDayPickerFragment extends ListFragment implements OnScrollListener {
  private static final String TAG = SimpleDayPickerFragment.class.getSimpleName();
  ...
  protected Runnable todayUpdater = new Runnable() {
    @Override
    public void run() {
  ...
  protected ScrollStateRunnable scrollStateChangedRunnable = new ScrollStateRunnable();
  ...
  protected void setUpAdapter() {
    ...
    weekParams.put(SimpleWeeksAdapter.WEEK_PARAMS_JULIAN_DAY, Time.getJulianDay(selectedDay.toMillis(false), selectedDay.gmtoff));
    if (adapter == null) {
      adapter = new SimpleWeeksAdapter(getActivity().getApplicationContext(), weekParams);
